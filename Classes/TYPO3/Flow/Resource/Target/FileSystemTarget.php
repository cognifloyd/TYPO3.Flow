...
class FileSystemTarget implements TargetInterface {
	...
	public function publishResource(Resource $resource, CollectionInterface $collection) {
		...
		if ($sourcePathAndFilename === FALSE) {
			throw new Exception(sprintf('Could not publish resource with SHA1 hash %s of collection %s because there seems to be no corresponding data in the storage.', $resource->getSha1(), $collection->getName()), 1375258146);
		}
	 ...
	 * @throws Exception
	 */
	protected function publishFile($sourcePathAndFilename, $relativeTargetPathAndFilename, $overwriteIfExists = FALSE) {
		...
		if (file_exists($targetPathAndFilename)) {
			if ($overwriteIfExists === FALSE) {
				$this->systemLogger->log(sprintf('FileSystemTarget: No need to re-publish, file already exists. (target: %s, file: %s)', $this->name, $relativeTargetPathAndFilename), LOG_DEBUG);
				return;
			}
			if (filemtime($sourcePathAndFilename) === filemtime($targetPathAndFilename)) {
				$this->systemLogger->log(sprintf('FileSystemTarget: No need to re-publish, file already exists and has same modification date / time. (target: %s, file: %s)', $this->name, $relativeTargetPathAndFilename), LOG_DEBUG);
				return;
			}
		}
		...
		$result = @copy($sourcePathAndFilename, $targetPathAndFilename);
		if ($result === FALSE) {
			throw new Exception(sprintf('Could not publish "%s" into resource publishing target "%s" because the source file could not be copied to the target location.', $sourcePathAndFilename, $this->name), 1375258399);
		}
	 ...
	 * @return string The relative path and filename, for example "c828d/0f88c/e197b/e1aff/7cc2e/5e86b/12442/41ac6/MyPicture.jpg"
	 */
